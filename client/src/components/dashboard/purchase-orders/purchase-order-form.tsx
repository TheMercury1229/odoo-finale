...
export function PurchaseOrderForm({ initialPo }: PurchaseOrderFormProps) {
  ...
  const handleReceiptExtracted = (data: ExtractedReceiptData) => {
    // 1. Vendor matching
    if (data.partyName && vendors.length > 0) {
      ...
      const matchedVendor =
        ...
        vendors.find((v) => {
          ...
          return words.some((w) => v.name.toLowerCase().includes(w));
        });

      if (matchedVendor) {
        setValue("vendorId", matchedVendor.id, {
          shouldValidate: true,
          shouldDirty: true,
        });
      }
      ...
      const newLines = data.lines.map((extractedLine) => {
        ...
        const unitPrice =
          extractedLine.unitPrice > 0
            ...
            : parseFloat(String(matchedProduct?.costPrice || 0)) || 0;

        return {
          productId: matchedProduct ? matchedProduct.id : "",
          analyticAccountId: null,
          quantity: Math.max(0.01, extractedLine.quantity || 1),
